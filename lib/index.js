...
const resolvers = {
	Query: {
		...
		allCategories: (_ ) => 
			generalRequest$1(`${URL}/${categoriesEntryPoint}`, 'GET'),
		// reviews query resolvers
		allReviews: (_, { page }) => {
			const params = page ? `?p=${page}` : '';
			return generalRequest$1(`${URL}/${reviewsEntryPoint + params}`, 'GET')
		},
		reviewById: (_, { ID }) =>
			generalRequest$1(`${URL}/${reviewsEntryPoint}/${ID}`, 'GET'),
		// posts query resolvers
		allPosts: (_, { page }) => {
			const params = page ? `?p=${page}` : '';
			return generalRequest$1(`${URL}/${postsEntryPoint + params}`, 'GET')
		},
		postById: (_, { ID }) =>
			generalRequest$1(`${URL}/${postsEntryPoint}/${ID}`, 'GET')
	},
	Mutation: {
		// reviews mutation resolvers
		createReview: (_, { review }) =>
			generalRequest$1(`${URL}/${reviewsEntryPoint}`, 'POST', review),
		updateReview: (_, { ID, review }) =>
			generalRequest$1(`${URL}/${reviewsEntryPoint}/${ID}`, 'PUT', review),
		deleteReview: (_, { ID, body }) =>
			generalRequest$1(`${URL}/${reviewsEntryPoint}/${ID}`, 'DELETE', body),
		// posts mutation resolvers
		createPost: (_, { post }) =>
			generalRequest$1(`${URL}/${postsEntryPoint}`, 'POST', post),
		updatePost: (_, { ID, post }) =>
			generalRequest$1(`${URL}/${postsEntryPoint}/${ID}`, 'PUT', post),
		deletePost: (_, { ID }) =>
			generalRequest$1(`${URL}/${postsEntryPoint}/${ID}`, 'DELETE'),
	}
};
...
const URL$1 = `http://${url}:${port$1}/${entryPoint}`;

const resolvers$1 = {
	Query: {
		historyByClientId: (_, { idCliente }) =>
			generalRequest(`${URL$1}/history/${idCliente}`, 'GET'),
	},
	Mutation: {
		createBill: (_, { idCliente }) =>
			generalRequest(`${URL$1}/bill`, 'POST', { "idCliente": 15,
